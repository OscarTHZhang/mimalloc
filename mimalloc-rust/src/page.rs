//! Phase 3 allocates and frees blocks from one single-threaded page.
//!
//! ```text
//! Page memory
//! +----------------+----------------+----------------+----------------+
...
//! allocated blocks: 1, free blocks: 3, total blocks: 4
//! ```
...
struct Page {
    ...
    block_count: usize,
    /// Number of blocks currently allocated to callers.
...
impl Page {
    ...
    /// Returns whether no blocks are currently allocated.
    fn is_empty(&self) -> bool {
        self.allocated_count == 0
    }

    /// Returns whether every block is currently allocated.
    ...
    /// Allocates and returns one block, or `None` when the page is full.
    fn allocate_block(&mut self) -> Option<NonNull<u8>> {
        let block = self.free_list.take()?;
        self.allocated_count += 1;
        debug_assert!(self.allocated_count <= self.block_count);
        Some(block)
    }

    /// Frees a block and returns it to the front of the free list.
    ///
    /// # Safety
    ///
    /// `block` must have been taken from this page, must no longer be accessed
    /// by the caller, and must not already be free.
    unsafe fn free_block(&mut self, block: NonNull<u8>) {
        debug_assert!(self.contains_block(block));
        debug_assert!(self.allocated_count > 0);
...
mod tests {
    ...
    fn divide_an_os_page_into_fixed_size_blocks() {
        ...
        unsafe {
            ...
            assert_eq!(page.allocate_block(), None);
    ...
    /// Checks that every block can be allocated and freed.
    ///
    /// A new page should be empty, become full after all blocks are allocated,
    /// and become empty again after every block is freed.
    #[test]
    fn allocate_and_free_blocks_on_one_page() {
        let mut page = Page::new(PAGE_SIZE, BLOCK_SIZE)
            .expect("the operating system should provide one allocator page");
        ...
        assert_eq!(page.free_count(), page.block_count());

        for block_index in 0..page.block_count() {
            assert_eq!(page.allocate_block(), page.block_ptr(block_index));
        }
        assert!(page.is_full());
        assert!(!page.is_empty());
        assert_eq!(page.allocated_count(), page.block_count());
        ...
        for block_index in 0..page.block_count() {
            ...
            // SAFETY: Every block was allocated exactly once and is not currently free.
            unsafe { page.free_block(block) };
        }
        assert!(page.is_empty());
        assert!(!page.is_full());
        assert_eq!(page.allocated_count(), 0);
